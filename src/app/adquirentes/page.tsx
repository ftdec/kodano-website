...
function BenefitItem({ text, delay = 0 }: { text: string; delay?: number }) {
  return (
    <motion.div
      ...
      transition={{ delay, duration: 0.5, ease: [0.22, 1, 0.36, 1] as const as const }}
    >
      <motion.div
        whileHover={{ x: 4 }}
...
function FlowStep({ number, text, delay = 0 }: { number: number; text: string; delay?: number }) {
  return (
    <motion.div
      variants={fadeInUp}
      initial="hidden"
        ...
        <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <motion.h1 
              ...
              animate={heroInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.8, delay: 0.2, ease: [0.22, 1, 0.36, 1] as const as const }}
            >
            ...
            <motion.p 
              ...
              initial={{ opacity: 0, y: 20 }}
              animate={heroInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.8, delay: 0.4, ease: [0.22, 1, 0.36, 1] as const as const }}
            >
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            {fluxo.map((passo, index) => (
              <FlowStep key={index} number={index + 1} text={passo} delay={index * 0.1} />
            ))}
          </motion.div>
        </div>
      </section>
